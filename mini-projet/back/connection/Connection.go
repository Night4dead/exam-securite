...
func ConnectSafe(w http.ResponseWriter, r *http.Request) {
	var info ConnetionInfo
	err := json.NewDecoder(r.Body).Decode(&info)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
	...
	log.Println("connection...")
	connectedUser := info.connectSafe()
...
func (info ConnetionInfo) connectSafe() *ConnectedUser {
	return securedQuery{info.Password}.GetResult()
}
